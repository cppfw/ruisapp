...
native_window::ios_egl_context_wrapper::ios_egl_context_wrapper(const utki::version_duplet& gl_version) :
    context([&](){
        ...
        if(gl_version.major >= 4){
            throw std::logic_error(utki::cat("ios_egl_context_wrapper::ios_egl_context_wrapper(): unknown OpenGL ES version requested: ", gl_version));
        ...
        if (!ctx) {
            throw std::runtime_error("ios_egl_context_wrapper::ios_egl_context_wrapper(): could not create OpenGL ES context");
        }
...
native_window::ios_view_controller_wrapper::ios_view_controller_wrapper(
    ...
    view_controller([&](){
        ...
        if(!vc){
            throw std::runtime_error("ios_view_controller_wrapper::ios_view_controller_wrapper(): could not create view controller");
        }
